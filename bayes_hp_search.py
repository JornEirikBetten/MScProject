...
def objective(learning_rate, depth, width):
    ...
    # Data splitting
    x_train, x_test, y_train, y_test = train_test_split(df, target, test_size=0.2, random_state=int(time.time()))

    y_train = y_train.to_numpy().reshape(len(y_train), 1); x_train = x_train.to_numpy()
    y_test = y_test.to_numpy().reshape(len(y_test), 1); x_test = x_test.to_numpy()
    ...
    for epoch in range(100):
        ...
        #rng.shuffle(indices)
        #batch_indices = np.array_split(indices, 20)
